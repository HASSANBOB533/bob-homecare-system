...
## New Features - Cancellation Policy & Notifications
- [x] Fix website unavailability issue (website is running correctly)
- [x] Display cancellation policy on booking form: "Non-refundable. Can be rescheduled if requested 24+ hours before service"
- [x] Display cancellation policy on check booking status page
- [x] Add reschedule functionality (change date/time) for bookings
- [x] Enforce 24-hour reschedule policy (only allow if booking is 24+ hours away)
- [x] Send notification to admin when customer cancels booking
- [x] Send notification to admin when customer reschedules booking
- [x] Admin notifications sent via Manus notification system
